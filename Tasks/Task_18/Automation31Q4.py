#=====================================================================================================
# Program Name     : DirectoryCopyExtensionAutomation
# Input            : Source directory, destination directory, file extension, time interval (CLI arguments)
# Output           : Copies files of a specified extension into a destination directory
#                    while preserving the original directory structure
# Description      : Periodically traverses a source directory and copies all files
#                    matching a given extension using os.walk and shutil.
...
def DirectoryCopyExtension(ExistDirName ,NewDirName ,ExtensionName ):
    ...
    print(Border)
    print(f"Files of {ExistDirName} with {ExtensionName} gets copied to {NewDirName} successfully")
    print(Border)
...
def main():
    ...
    if(len(sys.argv) == 2):
        ...
        if((sys.argv[1]== "--h") or (sys.argv[1]== "--H")):
            ...
            print("Copy files of given extension from source folder to destination folder ")

        elif((sys.argv[1]== "--u") or (sys.argv[1]== "--U")):
            print("Usage:")
            print("ScriptName.py SourceDirectory DestinationDirectory ExtensionName TimeInterval")
            print("SourceDirectory       : Name of Exsisting Directory")
            print("DestinationDirectory  : Name of Destination Directory")
            print("ExtensionName         : Name of File Extension that needs to be copied")
        ...
        print("ExtensionName : ",sys.argv[3])
        print("Time interval  :", interval, "minutes")

        schedule.every(interval).minutes.do(DirectoryCopyExtension, sys.argv[1], sys.argv[2],sys.argv[3])
